...
const res = await fetch("http://localhost:3000/api/approval", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ planId, step: Number(step), text: a.text, signature, deadline: a.deadline, nonce: a.nonce, guardian: a.guardian }) });
